...
'profile
Usage:
profile_trad -n <project_name> -b <batch_name> -t <input_type> -i <input_dir> -r <output_dir> -a <correlation> -p <plate_number> -o <operation> -l <norm_column> -v <norm_value> -c <no_cores> [-f <feat_list_file>]
...
-a <correlation> --correlation=<correlation>      Correlation method; e.g. pearsonr, mutinfo
...
profile.plate.location(pl = pl, project.name = proj.name, batch.name = batch.name, nrm.column = col.name, nrm.value = col.val, correlation = correlation, out.path = outp.dir, in.path = input.dir, in.type = input.type, cores = cores, feat.list = feat.list)